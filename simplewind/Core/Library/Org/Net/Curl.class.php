...
class Curl{
     ...
     */
    public function getRequest($url, $data = array(), $proxy = null, $expire = 30) {
     ...
     *
     * @param string $url     所要获取内容的网址
     * @param array  $post        所要提交的数据
     * @param array $extra   扩展
     * @param integer $timeout 时间限制
     * @param bool $ssl ssr
     * @return string
     *
     * @example
     ...
     */
    public static function send_http($rq) {

        $ch = curl_init();


        curl_setopt($ch, CURLOPT_URL, $rq['url']);
        switch (true) {
        ...
        //$header[] = 'User-Agent:'.Wanxiang::getUA();
        curl_setopt($ch, CURLOPT_USERAGENT, $_SERVER['HTTP_USER_AGENT']); // 模拟用户使用的浏览器
        ...
        isset($rq['timeout']) && curl_setopt($ch, CURLOPT_TIMEOUT, $rq['timeout']);
        isset($rq['data']) && in_array($method, array('POST', 'PUT')) && curl_setopt($ch, CURLOPT_POSTFIELDS, $rq['data']);
        $ssl = substr($rq['url'], 0, 8) == "https://" ? true : false;
        if( isset($rq['cert'])){
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER,true);
            curl_setopt($ch, CURLOPT_CAINFO, $rq['cert']);
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST,2);
            if (isset($rq['ssl_version'])) {
                curl_setopt($ch, CURLOPT_SSLVERSION, $rq['ssl_version']);
            } else {
                curl_setopt($ch, CURLOPT_SSLVERSION, 4);
            }
        }else if( $ssl ){
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER,false);   //true any ca
            curl_setopt($ch, CURLOPT_SSL_VERIFYHOST,1);       //check only host
            if (isset($rq['ssl_version'])) {
                curl_setopt($ch, CURLOPT_SSLVERSION, $rq['ssl_version']);
            } else {
                curl_setopt($ch, CURLOPT_SSLVERSION, 4);
