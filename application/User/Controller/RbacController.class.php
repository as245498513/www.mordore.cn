...
class RbacController extends AdminbaseController {
    ...
    public function _initialize() {
        parent::_initialize();
        $this->role_model = D("Common/MemberRole");
    }
    ...
    public function roledelete() {
        ...
        $role_user_model=M("MemberRoleUser");
     ...
     * 初始化后台菜单
     */
    private function initMemberMenu() {
        $Menu = F("MemberMenu");
        if (!$Menu) {
    ...
    public function authorize() {
        $this->auth_access_model = D("Common/MemberAuthAccess");
       //角色ID
    ...
    public function authorize_post() {
    	...
    	if (IS_POST) {
    		...
    		if (is_array($_POST['menuid']) && count($_POST['menuid'])>0) {
    			...
    			$menu_model=M("MemberMenu");
    			$auth_rule_model=M("MemberAuthRule");
