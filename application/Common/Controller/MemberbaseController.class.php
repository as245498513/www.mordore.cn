...
class MemberbaseController extends AppframeController{

	public function __construct() {
	    
		$admintpl_path=C("SP_MEMBER_TMPL_PATH").C("SP_MEMBER_DEFAULT_THEME")."/";
		C("TMPL_ACTION_SUCCESS",$admintpl_path.C("SP_MEMBER_TMPL_ACTION_SUCCESS"));
		C("TMPL_ACTION_ERROR",$admintpl_path.C("SP_MEMBER_TMPL_ACTION_ERROR"));
		parent::__construct();
		$time=time();
	...
	function _initialize(){
		parent::_initialize();
       define("TMPL_PATH", C("SP_MEMBER_TMPL_PATH"));
		$this->load_app_admin_menu_lang();//暂时取消后台多语言
       $session_member_id=session('MEMBER_ID');
    	if(!empty($session_member_id)){
    		$users_obj= M("Members");
    		$user=$users_obj->where(array('id'=>$session_member_id))->find();
    		if(!$this->check_access($session_member_id)){
				$this->error("您没有访问权限！");
			}
			$this->assign("admin",$user);
		}else{
			if(IS_AJAX){
				$this->error("您还没有登录！",U("user/public/login"));
			}else{
				header("Location:".U("user/public/login"));
				exit();
			}
	...
	public function initMenu() {
		$session_member_id=session('MEMBER_ID');
		F("MemberMenu".$session_member_id,"");
		$Menu = F("MemberMenu".$session_member_id);
		if (!$Menu) {
			$Menu=D("Common/MemberMenu")->menu_cache();
		}
		return $Menu;
	}
	...
	public function parseTemplate($template='') {
		$tmpl_path=C("SP_MEMBER_TMPL_PATH");
		define("SP_TMPL_PATH", $tmpl_path);
    	if($this->theme) { // 指定模板主题
    	    $theme = $this->theme;
    	}else{
    	    // 获取当前主题名称
    	    $theme      =    C('SP_MEMBER_DEFAULT_THEME');
    	}
	...
	private function check_access($uid){
		...
		$no_need_check_rules=array("UserIndexindex","UserMainindex");

		if( !in_array($rule,$no_need_check_rules) ){
			return sp_member_auth_check($uid);
		}else{
			return true;
		}
	}
