...
class TagLibSpadmin extends TagLib {
    ...
    protected $tags = array(
        ...
        'usertpl' => array("attr" => "file", "close" => 0),
    );
     ...
     * 模板包含标签 
     * 格式
     * <usertpl file="APP/模块/模板"/>
     * @staticvar array $_admintemplateParseCache
     * @param type $attr 属性字符串
     * @param type $content 标签内容
     * @return array 
     */
    public function _usertpl($tag, $content) {
        $file = $tag['file'];
        $counts = count($file);
        if ($counts < 3) {
            $file_path = "User" .  "/" . $tag['file'];
        } else {
            $file_path = $file[0] . "/" . "Tpl" . "/" . $file[1] . "/" . $file[2];
        }
        //模板路径
        $TemplatePath = sp_add_template_file_suffix( C("SP_MEMBER_TMPL_PATH") .C("SP_MEMBER_DEFAULT_THEME")."/". $file_path );
